...
class OneStagePromptManager(object):
    def __init__(self, args):
        ...
        self.other_nodes_list = [[] for _ in range(self.args.batch_size)]
        self.node_imgs = [[] for _ in range(self.args.batch_size)]
        self.other_imgs = [[] for _ in range(self.args.batch_size)]
        self.graph  = [{} for _ in range(self.args.batch_size)]
        self.other_graph  = [{} for _ in range(self.args.batch_size)]
        self.trajectory = [[] for _ in range(self.args.batch_size)]
        self.other_trajectory = [[] for _ in range(self.args.batch_size)]
    ...
    def make_map_prompt(self, i):
        ...
        if self.merge_flag:
            common_nodes = set(self.nodes_list[i]) & set(self.other_nodes_list[i])

            for other_node in common_nodes:
                main_cache_index = []
                aux_cache_index = []

                if other_node in trajectory:
                    continue
                if other_node in self.other_trajectory[i] and other_node not in no_dup_nodes_other:
                    no_dup_nodes_other.append(other_node)
                    node_index = self.nodes_list[i].index(other_node)

                    adj_text = ''
                    adjacent_nodes = self.other_graph[i][other_node]
                    for adj_node in adjacent_nodes:
                        if adj_node in self.nodes_list[i]:
                            adj_index = self.nodes_list[i].index(adj_node)
                            if adj_index in main_cache_index:
                                continue
                            adj_text += f""" {adj_index},"""
                            main_cache_index.append(adj_index)
                        else:
                            adj_index = self.other_nodes_list[i].index(adj_node)
                            if adj_index in aux_cache_index:
                                continue
                            adj_text += f""" other_{adj_index},"""
                            aux_cache_index.append(adj_index)
                            other_index.append(adj_index)
                    other_graph_text += f"""\nPlace {node_index} is connected with Places{adj_text}"""[:-1]
                
                if other_node not in self.other_trajectory[i] and other_node not in no_dup_nodes_other:
                    no_dup_nodes_other.append(other_node)
                    node_index = self.nodes_list[i].index(other_node)
                    adj_text = ''

                    for j in self.trajectory[i]:
                        if other_node in self.graph[i][j]:
                            adj_index = self.nodes_list[i].index(j)
                            if adj_index in main_cache_index:
                                continue
                            adj_text += f""" {adj_index},"""
                            main_cache_index.append(adj_index)
                    for j in self.other_trajectory[i]:
                        if other_node in self.other_graph[i][j]:
                            adj_index = self.other_nodes_list[i].index(j)
                            if adj_index in aux_cache_index:
                                continue
                            adj_text += f""" other_{adj_index},"""
                            aux_cache_index.append(adj_index)
        ...
        for i in set(other_index):
            graph_supp_text += f"""\nPlace other_{i}, which is corresponding to Image other_{i}"""

        if supp_exist is None:
            graph_supp_text = """Nothing yet."""
        
        if self.merge_flag is False:
            other_graph_text = """Nothing yet."""
        
        if len(set(other_index)) == 0:
            other_graph_text = """Nothing yet."""

        return trajectory_text, graph_text, other_graph_text, set(other_index), graph_supp_text

    def make_r2r_json_prompts(self, obs, cand_inputs, t, other_prompt):
        ...
        + """ Images prefixed with 'Image other_*' denote locations derived from the other agents' trajectories or observations. (note that these may not always be present)"""
        ...
        map_supo_info = """'Supplementary_Map' refers to the connectivity between the places you have not explored, which shared intelligence from other agents, (note that Supplementary_Map may not always be present). It includes unexplored areas , effectively expanding your situational awareness. Use this information to refine your navigation strategy and explore beyond your direct line of sight. Places prefixed with 'other_*' denote locations derived from the other agents' trajectories or observations."""
        ...
        if self.merge_flag:
            self.other_imgs[0] = other_prompt.node_imgs[0]
            self.other_nodes_list[0] = other_prompt.nodes_list[0]
            self.other_graph[0] = other_prompt.graph[0]
            self.other_trajectory[0] = other_prompt.trajectory[0]
        ...
        for i in range(batch_size):
            ...
            trajectory_text, graph_text, other_graph_text, other_index, graph_supp_text = self.make_map_prompt(i)

            if t == 0:
                prompt = f"""Instruction: {instruction}\nHistory: {init_history}\nTrajectory: {trajectory_text}\nMap:{graph_text}\nSupplementary Info: {graph_supp_text}\nPrevious Planning:\n{self.planning[i][-1]}\nAction options (step {str(t)}): {action_options_batch[i]}"""
            else:
                prompt = f"""Instruction: {instruction}\nHistory: {self.history[i]}\nTrajectory: {trajectory_text}\nMap:{graph_text}\nSupplementary_Map:{other_graph_text}\nSupplementary Info: {graph_supp_text}\nPrevious Planning:\n{self.planning[i][-1]}\nAction options (step {str(t)}): {action_options_batch[i]}"""

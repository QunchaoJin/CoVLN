...
from vln.data_utils import construct_instrs, pair_episodes
...
def build_dataset(args, rank=0, is_test=True):
    ...
    if 'processed' in split:
        ...
        val_instr_data, val_instr_data_au = pair_episodes(val_instr_data, pairing=args.pairing)

    else:
        val_instr_data, val_instr_data_au = construct_instrs(
            args.anno_dir, args.dataset, [split],
            tokenizer=args.tokenizer, max_instr_len=args.max_instr_len,
            is_test=is_test, pairing=args.pairing
        )

    # --start / --end index the paired episodes: each index is one (main, aux) pair
    if args.end is not None:
        val_instr_data = val_instr_data[args.start:args.end]
        val_instr_data_au = val_instr_data_au[args.start:args.end]
    print(f'------------------ Evaluate {len(val_instr_data)} pairs in {split} (pairing={args.pairing}) ------------------')
    ...
    val_env_au = dataset_class(
        val_instr_data_au, args.connectivity_dir, batch_size=args.batch_size,
        seed=args.seed+rank,
        name=split, args=args,
    )   # evaluation using all objects


    val_envs[split] = (val_env, val_env_au)
...
def valid(args, val_envs, rank=0):
    ...
    agent = agent_class(args, list(val_envs.values())[0][0], list(val_envs.values())[0][1], rank=rank)
